  ...
  <div>
    ...
    <div class="content-list-box">
      ...
      <div class="top">
        {{visible}}
          <ul>
            <a @click="setId(item.user_id,item.user_name,item.user_head)">
              <!--头像 用户名 时间  -->
              <li class="head-img">
...
export default{
  props: ['item', 'folBoxVisible'],   // folBoxVisible: true  follow 盒子 若用户自己点开自己的文章 则不显示
  data () {
    return {
      err: '',
      post: '',
  ...
  computed: {
    ...
    visible: function () {
      if (this.hasLogin) {
        ...
        // console.log('------------')
        if (this.uinfo.user_id !== this.item.user_id) {
          axios.get('api/onearticle', {
            params: {
              user_id: this.uinfo.user_id,
              concern_user_id: this.item.user_id
            }
          }).then((response) => {
  ...
  methods: {
    ...mapMutations(['setCurComment', 'clearOrdInfo', 'setCurId']),
    // 设置目前点击微博所需要显示的评论及id
